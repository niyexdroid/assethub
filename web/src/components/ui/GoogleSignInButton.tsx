...
export function GoogleSignInButton({ label = 'Continue with Google', onError }: Props) {
  ...
  return (
    <button
      ...
      onClick={() => login()}
      disabled={loading}
      className="w-full h-11 rounded-xl border border-border bg-card text-foreground font-medium text-sm hover:bg-muted transition-colors disabled:opacity-50 flex items-center justify-center gap-2.5"
    >
