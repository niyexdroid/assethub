...
Create the tenant TenancyDetail page at `web/src/pages/tenant/TenancyDetail.tsx` — comprehensive tenancy information view with payment history and terminate action.

## What to Build

A React component that:
1. Fetches tenancy by ID from URL params + payment history
2. Shows status badge (active/pending/terminated/declined) with colors
3. Shows info cards: property (linked), landlord contact
4. Shows rent & dates section (amount, type, caution fee, agency fee, start/end dates)
5. Shows agreement status (tenant/landlord signed indicators)
6. Shows payment history filtered for this tenancy
7. Has terminate button with confirmation modal (for active tenancies)
8. Back link to /tenancy
...
2. Commit with: `feat: add tenant Tenancy Detail page with payment history and terminate`
...
- Uses `ErrorState`, `EmptyState` from `@/components/custom/`
